 ...
 * The tree can be modified by adding and removing labels.
 ...
 *   "nonconfig" and "show" can be added by using, eg
 * cmd @datamodel, @add:show, @add:nonconfig, callback();
	      ...
	      cbuf         *cb)
{
	...
	if (1) { 
#else
		if (!show_tree || key_leaf) { /* XXXY show */
#endif
	      ...
	      cbuf         *cb)
{
	...
	*/
    ...
    if (last_key){
#else
        if (last_key && (show_tree||extralevel)) { /* XXXY show */
#endif
	cprintf(cb, "%*s}\n", level*3, "");
    }
    retval = 0;
 ...
 * 1. Add "show" label on all empty children of LIST
 * 2. Add "nonconfg" label on nodes which have YANG "config false" as children
 * 3. Add "leaf" label on nodes which are YANG non-ley LEAFs
 * @param[in]  h   Clixon handle
    	      ...
    	      yang_stmt    *ykeyparent)
{
	...
	if (co->co_type == CO_EMPTY){
	    if (ykeyparent != NULL){
		if (yang_keyword_get(ykeyparent) == Y_LIST){
		    /* list has a <cr> which is marked as "show" */
		    if ((co->co_cvec = cvec_add_name(co->co_cvec, "show")) == NULL)
			goto done;
		}
	    }
	    else
	...
	if ((yc = yang_find_datanode(y, co->co_command)) == NULL)
	    continue;
	/* If leaf add "leaf" label for non-leaf keys */
	if (yang_keyword_get(yc) == Y_LEAF && ykeyparent != NULL){
	    if ((co->co_cvec = cvec_add_name(co->co_cvec, "leaf")) == NULL)
		goto done;
	}
	...
	if (yang_keyword_get(y) == Y_LIST && yang_key_match(y, co->co_command)){
	    if (yang2cli_post(h, co_pt_get(co), 0, y, yc) < 0)
		goto done;
	}
	else if (yang2cli_post(h, co_pt_get(co), 0, yc, NULL) < 0)
	    goto done;
    }
    retval = 0;
 done:
 ...
 * @param[in]  show_tree Is tree for show cli command
 * @param[out] pt        CLIgen parse-tree (must exist on input)
 * @retval     0         OK
 * @retval    -1         Error
 */
int
	 ...
	 parse_tree        *pt)
{
	...
	if (e < nexvec)
	    continue;
	if (yang2cli_stmt(h, yc, gt, 0, state,
#if 1
			  1,
#else
			  show_tree,
			  ...
			  cb) < 0)
	    goto done;
	if ((prefix = yang_find_myprefix(yc)) == NULL){
	    clicon_err(OE_YANG, 0, "Module %s lacks prefix (shouldnt happen)", yang_argument_get(yc));
	    goto done;
	}
	...
	for (i=j; i<pt_len_get(pt); i++){
	    if ((co = pt_vec_i_get(pt, i)) != NULL)
		co_prefix_set(co, prefix);
	}
	/* Post-processing, iterate over the generated cligen parse-tree with corresponding yang
	 * Assume appended. XXX otherwise need to mark
	 */
	if (yang2cli_post(h, pt, j, yc, NULL) < 0)
	    goto done;

	j = i; /* assume appended. XXX otherwise need to mark */

	cbuf_reset(cb);
    }
    cligen_parsetree_sort(pt, 0);
...
/*! Init yang2cli, in particular @datamodel tree generation
 *
 * Initialize CLIgen generation from YANG models.
 * Add "nonconfig" and "show" as labels that are by default removed from when applying @datamodel.
 *
 * @param[in]  h      Clixon handle
 */
int
yang2cli_init(clicon_handle h)
{
    ...
    cvv = cligen_reftree_filter_get(cli_cligen(h));
    if ((cvv = cvec_add_name(cvv, "nonconfig")) == NULL)
	goto done;
    if ((cvv = cvec_add_name(cvv, "show")) == NULL)
    	goto done;
    if (cligen_reftree_filter_set(cli_cligen(h), cvv) < 0)
	goto done;
    cvv = NULL; /* Consumed by set fn ^*/
    retval = 0;
  done:
    return retval;
}
